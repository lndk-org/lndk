...
impl Default for DNSResolverMessageHandler {
    fn default() -> Self {
        Self::new()
    }
}

impl DNSResolverMessageHandler {
    pub fn new() -> Self {
    ...
    pub async fn resolve_hrn_to_offer(&self, name_str: &str) -> Result<String, OfferError> {
    ...
    pub async fn resolve_locally(&self, name: String) -> Result<String, OfferError> {
        ...
        let resolution = tokio::time::timeout(
            ...
            self.resolver.resolve_hrn(&hrn_parsed),
        )
        .await
        .map_err(|_| {
            OfferError::HrnResolutionFailure(format!(
                "{name}: resolution timed out after {DEFAULT_DNS_TIMEOUT_SECS}s"
            ))
        })?
...
mod tests {
    ...
    fn test_extract_offer_from_simple_uri() {
        let handler = DNSResolverMessageHandler::new();
