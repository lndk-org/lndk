...
enum Commands {
    ...
    /// GetInvoice fetch an invoice and return it from a BOLT 12 offer, provided as a
    /// 'lno'-prefaced offer string.
    GetInvoice {
        /// The offer string.
        offer_string: String,
        /// Amount the user would like to pay. If this isn't set, we'll assume the user is paying
    ...
    match args.command {
        ...
        Commands::GetInvoice {
            ...
            match client.get_invoice(request).await {
                Ok(response) => println!("Invoice: {:?}.", response.get_ref().invoice),
                Err(err) => println!("Error getting invoice for offer: {err:?}"),
            };
        }
        Commands::PayInvoice { ref invoice_string } => {
            let tls = read_cert_from_args(&args);
            let grpc_host = args.grpc_host.clone();
            let grpc_port = args.grpc_port;
            let channel = Channel::from_shared(format!("{grpc_host}:{grpc_port}")) //
            ...
            match client.pay_invoice(request).await {
                Ok(_) => println!("Successfully paid for offer!"),
                Err(err) => println!("Error paying invoice: {err:?}"),
            };
        }
    }
}
